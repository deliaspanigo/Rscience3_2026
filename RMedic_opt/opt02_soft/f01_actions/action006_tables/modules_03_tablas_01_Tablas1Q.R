## Segmento del UI
modules_03_tablas_01_Tablas1Q_UI <- function(id) {
  ns <- NS(id)
...
## Segmento del server
modules_03_tablas_01_Tablas1Q_SERVER <- function(input, output, session, 
                            minibase, 
                            batalla_naval,
                            decimales) {
  ...
  check_ok <- reactive({
    ...
    req(batalla_naval()[[4]], batalla_naval()[[4]] == 1) # 1Q = 1
  ...
  observeEvent(list(Reactive_tabla_1q_RMedic(), cantidad_tablas()), {
