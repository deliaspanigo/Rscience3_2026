## Segmento del UI
modules_03_tablas_04_Tablas2C_UI <- function(id) {
  ns <- NS(id)
...
## Segmento del server
modules_03_tablas_04_Tablas2C_SERVER <- function(input, output, session, 
                            minibase, 
                            batalla_naval,
                            decimales) {
  ...
  check_ok <- reactive({
    ...
    req(batalla_naval()[[4]], batalla_naval()[[4]] == 4) # CC es 4
  ...
  output$SeccionTablas2C <- renderUI({
    ...
    div(
      h2_mod("RMedic - Tablas para 2 Variables Numéricas"),
