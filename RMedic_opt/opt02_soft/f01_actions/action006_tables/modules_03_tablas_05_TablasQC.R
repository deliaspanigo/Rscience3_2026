## Segmento del UI
modules_03_tablas_05_TablasQC_UI <- function(id) {
  ns <- NS(id)
...
## Segmento del server
modules_03_tablas_05_TablasQC_SERVER <- function(input, output, session, 
                            minibase, 
                            batalla_naval,
                            decimales) {
  ...
  check_ok <- reactive({
    ...
    req(batalla_naval()[[4]], batalla_naval()[[4]] == 5) # QC es 5
  ...
  output$SeccionTablasQC <- renderUI({
    ...
    div(
      h2_mod("RMedic - Tablas para 1 Variable Categórica y 1 Variable Numérica, o viceversa"),
